...
class iris_gripper():
    ...
    def __init__(self):
        ...
        self.box_model_name = 'strawberry_box'
        self.drone_model_name = 'iris'
        self.box_index = 0
        self.drone_index = 0
    ...
    def check(self):
        ...
        if (self.box_index != -1 and self.drone_index !=-1 ):
            if(abs(self.drone_coordinates[0] - self.box_coordinates[0]) < 0.15 and abs(self.drone_coordinates[1] - self.box_coordinates[1]) < 0.15 and abs(self.drone_coordinates[2] - self.box_coordinates[2])<0.25):
...
def main():
    iris_gripper_obj = iris_gripper()
    r = rospy.Rate(10)
    while not rospy.is_shutdown():
        try:
            iris_gripper_obj.check()
            r.sleep()
